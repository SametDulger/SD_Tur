...
namespace SDTur.Infrastructure.Repositories
{
    public class TicketRepository : Repository<Ticket>, ITicketRepository
    {
        ...
        public async Task<string> GenerateTicketNumberAsync()
        {
            ...
            var lastNumber = int.Parse(lastTicket.TicketNumber.Substring(1));
            return $"A{(lastNumber + 1):D8}";
        }
        ...
        public async Task<IEnumerable<Ticket>> GetTicketsForBusAssignmentAsync(int tourScheduleId)
        {
            return await _dbSet
                .Include(t => t.Hotel)
                .Include(t => t.Hotel.Region)
                .Where(t => t.TourScheduleId == tourScheduleId && !t.IsCancelled && t.RequiresService)
                .OrderByDescending(t => t.Hotel.Region.DistanceFromKemer)
                .ThenBy(t => t.Hotel.Order)
                .ToListAsync();
        }
