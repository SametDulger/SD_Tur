...
namespace SDTur.Web.Controllers.Tour
{
    public class CustomerDistributionsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("TourScheduleId,BusId,TicketId,EmployeeId,DistributionDate,Status,Notes")] CreateCustomerDistributionViewModel createCustomerDistributionViewModel)
        {
            if (ModelState.IsValid)
            {
                await _apiService.PostAsync<CreateCustomerDistributionViewModel, CustomerDistributionViewModel>("api/customerdistributions", createCustomerDistributionViewModel);
