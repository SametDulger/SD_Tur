...
namespace SDTur.Web.Controllers.Tour
{
    public class TourReportsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("TourScheduleId,ReportType,ReportDate,StartDate,EndDate,TotalCustomers,FullPriceCustomers,HalfPriceCustomers,GuestCustomers,TotalIncome,TotalExpense,NetProfit,Currency,ReportData,Status,EmployeeId")] CreateTourReportViewModel createTourReportViewModel)
        {
            if (ModelState.IsValid)
            {
                await _apiService.PostAsync<CreateTourReportViewModel, TourReportViewModel>("api/tourreports", createTourReportViewModel);
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,TourScheduleId,ReportType,ReportDate,StartDate,EndDate,TotalCustomers,FullPriceCustomers,HalfPriceCustomers,GuestCustomers,TotalIncome,TotalExpense,NetProfit,Currency,ReportData,Status,EmployeeId,IsActive")] UpdateTourReportViewModel updateTourReportViewModel)
        {
            ...
            if (ModelState.IsValid)
            {
                await _apiService.PutAsync<UpdateTourReportViewModel, TourReportViewModel>($"api/tourreports/{id}", updateTourReportViewModel);
                return RedirectToAction(nameof(Index));
            }
